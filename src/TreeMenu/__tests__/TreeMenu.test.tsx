...
describe('TreeMenu', () => {
  ...
  it('should work with an alternative key separator', () => {
    ...
    const targetNodeWithAlternativeKey = wrapper
      .findWhere(node => node.key() === 'item2/a;b')
      .get(0);
    expect(targetNodeWithAlternativeKey).not.toBeUndefined();
    const targetNodeWithAlternativeKey2 = wrapper
      .findWhere(node => node.key() === 'item2/a;b;b/2')
      .get(0);
    expect(targetNodeWithAlternativeKey2).not.toBeUndefined();
    const targetNodeWithDefaultKey = wrapper
      .findWhere(node => node.key() === 'item2/a/b')
      .get(0);
